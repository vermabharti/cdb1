...
class LeafSubMenu extends StatefulWidget {
  final String mid;
  final String id;
  final String title;
  LeafSubMenu(
      {Key key, @required this.id, @required this.mid, @required this.title})
      : super(key: key);
...
class _MyPageState extends State<LeafSubMenu> {
  // leaf Menu part With URL and TITLE
  _fetchSubMenu() async {
    var connectivityResult = await (Connectivity().checkConnectivity());
    if (connectivityResult == ConnectivityResult.mobile ||
        connectivityResult == ConnectivityResult.wifi) {
      final formData = jsonEncode({
        "primaryKeys": ["${widget.mid}", "${widget.id}"]
      });
      Response response = await ioClient.post(LEAF_SUB_MENU_URL,
